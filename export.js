...
function populateLandmarkData(formattedData, prefix, landmarks, count) {
    ...
    // Set all to 'null' if landmarks array is empty
...
function exportData(firebaseUrl, adminSdkPath) {
    // remove quotations and change backward slashes to forward slashes
    const updatedPath = adminSdkPath.replace(/\\/g, '/').replace(/"/g, '');
  
    const serviceAccount = require(updatedPath);
    admin.initializeApp({
    ...
    const ref = db.ref('/Experimental_Data');
    ...
    return new Promise((resolve, reject) => {
        ...
        ref.once('value', (snapshot) => {
            ...
            if (!data) {
                ...
                // fucntion to generate landmark headers
